  ...
  const handleItemChange = (index: number, field: string, value: any) => {
    ...
    if (field === 'product_id') {
      ...
      if (product && product.standard_rate) {
        ...
        newItems[index].amount = (newItems[index].quantity || 0) * product.standard_rate;
      }
    }
        ...
        <CardContent>
          <Box sx={{ display: 'flex', gap: 2, alignItems: 'center' }}>
            ...
            <Box sx={{ ml: 'auto', display: 'flex', gap: 2 }}>
              <Chip label={`Total Sales: ${sales.length}`} color="primary" />
              <Chip
                label={`Total Amount: ₹${sales.reduce((sum, s) => sum + s.total_amount, 0).toLocaleString()}`}
                color="success"
              />
            </Box>
          </Box>
